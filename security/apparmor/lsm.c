...
static int apparmor_sk_alloc_security(struct sock *sk, int family, gfp_t flags)
{
	...
	memcpy(SK_CTX(sk), ctx, sizeof(struct aa_sk_ctx));
...
{
	struct aa_sk_ctx *ctx = (struct aa_sk_ctx *)&SK_CTX(sk);

	memset(SK_CTX(sk), 0, sizeof(struct aa_sk_ctx));
...
{
	struct aa_sk_ctx *ctx = (struct aa_sk_ctx *)&SK_CTX(sk);
	struct aa_sk_ctx *new = (struct aa_sk_ctx *)&SK_CTX(newsk);
				       ...
				       int type, int protocol, int kern)
{
	...
	if (sock->sk) {
		struct aa_sk_ctx *ctx = (struct aa_sk_ctx *)&SK_CTX(sock->sk);
